...
export default class Datatable extends LightningElement {
    ...
    set sObject(value) {
        ...
        if (this.enableLiveUpdates) {
            getPushTopic({ sObjectApiName: value }).then((channelName) => {
    ...
    handleSave(event) {
        ...
        let updatePromises = event.detail.draftValues.map((row) => {
            ...
            for (let key of Object.keys(row)) {
                // Replace fieldName with editFieldName before performing the update
            ...
            return updateRecord({ fields: row })
                ...
                .catch((error) => {
                    ...
                    const rowErrorMessages =
                        (error.body.output.errors && error.body.output.errors.map(formatError)) || [];
                    const fieldErrors = Object.values(error.body.output.fieldErrors).reduce(
    ...
    getRowValue(recordId) {
        let filter = this.where + (this.where ? ' AND ' : ' WHERE ') + `Id='${recordId}'`;
